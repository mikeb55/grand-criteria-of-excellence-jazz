"""
Chromatic Orbit x5 - John Scofield Style (5-STAR VERSION)
==========================================================

UPGRADES FOR 5-STAR:
- More angular chromatic melody (Scofield's DNA)
- Funkier rhythms with anticipations
- More sophisticated quartal stacking
- Grittier triad pairs with altered dominants

Key: Bb7 (funky dominant)
Tempo: 112 BPM (grooving)
"""

import xml.etree.ElementTree as ET
from xml.dom import minidom
from pathlib import Path
from dataclasses import dataclass
from typing import List, Tuple

NOTE_TO_MIDI = {
    "C": 0, "C#": 1, "Db": 1, "D": 2, "D#": 3, "Eb": 3,
    "E": 4, "F": 5, "F#": 6, "Gb": 6, "G": 7, 
    "G#": 8, "Ab": 8, "A": 9, "A#": 10, "Bb": 10, "B": 11
}

MIDI_TO_NOTE = {
    0: ("C", 0), 1: ("C", 1), 2: ("D", 0), 3: ("E", -1),
    4: ("E", 0), 5: ("F", 0), 6: ("F", 1), 7: ("G", 0),
    8: ("A", -1), 9: ("A", 0), 10: ("B", -1), 11: ("B", 0)
}

WHOLE = 64
DOTTED_HALF = 48
HALF = 32
DOTTED_QUARTER = 24
QUARTER = 16
DOTTED_EIGHTH = 12
EIGHTH = 8
SIXTEENTH = 4
BAR = 64


@dataclass
class Note:
    pitch: int
    duration: int
    is_rest: bool = False
    
    def get_xml(self) -> Tuple[str, int, int]:
        if self.is_rest:
            return ("C", 4, 0)
        octave = (self.pitch // 12) - 1
        pc = self.pitch % 12
        step, alter = MIDI_TO_NOTE[pc]
        return (step, octave, alter)


@dataclass
class ChordVoicing:
    pitches: List[int]
    duration: int


@dataclass 
class Chord:
    root: str
    quality: str
    
    def to_kind(self) -> str:
        kinds = {
            "7": "dominant", "9": "dominant-ninth", "7#9": "dominant",
            "m7": "minor-seventh", "m9": "minor-ninth",
            "7sus4": "suspended-fourth", "7alt": "dominant",
            "": "major", "m": "minor",
        }
        return kinds.get(self.quality, "dominant")


def verify(items, bar_num, section):
    total = sum(i.duration for i in items)
    ok = "✓" if total == BAR else "✗"
    print(f"  {ok} {section} Bar {bar_num}: {total}")
    return total == BAR


class ChromaticOrbitX5:
    """5-star John Scofield inspired composition."""
    
    def __init__(self):
        self.title = "Chromatic Orbit x5"
        self.composer = "GCE (John Scofield Style)"
        self.tempo = 112
        self.divisions = 16
        self.key_fifths = -2  # Bb
        
    def p(self, n, o):
        return NOTE_TO_MIDI[n] + (o + 1) * 12
    
    def n(self, note, oct, dur):
        return Note(self.p(note, oct), dur)
    
    def r(self, dur):
        return Note(0, dur, is_rest=True)
    
    def chord(self, notes, dur):
        return ChordVoicing([self.p(n, o) for n, o in notes], dur)
    
    # =========================================================================
    # INTRO: Angular Scofield melody with chromatic DNA
    # =========================================================================
    
    def intro_melody(self):
        items = []
        
        # Bar 1: Bb7 - Angular opening with chromatic approach
        # 8+8+8+8+8+8+16 = 64 ✓
        bar1 = [
            self.n("Bb", 4, EIGHTH),     # Root
            self.n("D", 5, EIGHTH),      # 3rd
            self.n("F", 5, EIGHTH),      # 5th
            self.n("E", 5, EIGHTH),      # Chromatic approach!
            self.n("Ab", 5, EIGHTH),     # b7
            self.n("G", 5, EIGHTH),      # 6th
            self.n("F", 5, QUARTER),     # 5th - funky landing
        ]
        verify(bar1, 1, "Intro")
        items.extend(bar1)
        
        # Bar 2: Continue angular motion
        # 8+8+16+8+8+16 = 64 ✓
        bar2 = [
            self.n("Bb", 5, EIGHTH),     # Root high
            self.n("A", 5, EIGHTH),      # Chromatic!
            self.n("Ab", 5, QUARTER),    # b7
            self.n("G", 5, EIGHTH),      # 6th
            self.n("Gb", 5, EIGHTH),     # Chromatic tension!
            self.n("F", 5, QUARTER),     # 5th resolve
        ]
        verify(bar2, 2, "Intro")
        items.extend(bar2)
        
        # Bar 3: Eb7 - IV chord with Scofield bite
        # 8+8+8+8+16+16 = 64 ✓
        bar3 = [
            self.n("Eb", 5, EIGHTH),     # Root
            self.n("G", 5, EIGHTH),      # 3rd
            self.n("Db", 5, EIGHTH),     # b7
            self.n("C", 5, EIGHTH),      # Chromatic
            self.n("Bb", 4, QUARTER),    # 5th
            self.n("G", 4, QUARTER),     # 3rd low
        ]
        verify(bar3, 3, "Intro")
        items.extend(bar3)
        
        # Bar 4: Bb7 - Rhythmic anticipation
        # 8+8+16+32 = 64 ✓
        bar4 = [
            self.r(EIGHTH),              # Syncopation!
            self.n("F", 5, EIGHTH),      # Anticipate
            self.n("D", 5, QUARTER),     
            self.n("Bb", 4, HALF),       
        ]
        verify(bar4, 4, "Intro")
        items.extend(bar4)
        
        # Bar 5: Gm7 - Minor area with edge
        # 8+8+8+8+8+8+16 = 64 ✓
        bar5 = [
            self.n("G", 4, EIGHTH),      # Root
            self.n("Bb", 4, EIGHTH),     # b3
            self.n("C", 5, EIGHTH),      # 4th (sus feel)
            self.n("D", 5, EIGHTH),      # 5th
            self.n("Db", 5, EIGHTH),     # Chromatic!
            self.n("C", 5, EIGHTH),      # Back
            self.n("Bb", 4, QUARTER),    
        ]
        verify(bar5, 5, "Intro")
        items.extend(bar5)
        
        # Bar 6: Cm7 - Deeper
        # 8+8+16+16+16 = 64 ✓
        bar6 = [
            self.n("C", 5, EIGHTH),      
            self.n("Eb", 5, EIGHTH),     
            self.n("G", 5, QUARTER),     
            self.n("F", 5, QUARTER),     # 11th
            self.n("Eb", 5, QUARTER),    
        ]
        verify(bar6, 6, "Intro")
        items.extend(bar6)
        
        # Bar 7: F7#9 - Altered dominant BITE!
        # 8+8+8+8+16+16 = 64 ✓
        bar7 = [
            self.n("A", 4, EIGHTH),      # 3rd
            self.n("Eb", 5, EIGHTH),     # b7
            self.n("G#", 5, EIGHTH),     # #9!!! Scofield!
            self.n("F", 5, EIGHTH),      # Root
            self.n("E", 5, QUARTER),     # Chromatic resolve
            self.n("C", 5, QUARTER),     
        ]
        verify(bar7, 7, "Intro")
        items.extend(bar7)
        
        # Bar 8: Bb7 - Return with punch
        # 8+8+16+32 = 64 ✓
        bar8 = [
            self.n("D", 5, EIGHTH),      
            self.n("F", 5, EIGHTH),      
            self.n("Ab", 5, QUARTER),    # b7
            self.n("Bb", 4, HALF),       # Root - hold
        ]
        verify(bar8, 8, "Intro")
        items.extend(bar8)
        
        return items
    
    def intro_bass(self):
        notes = []
        notes.extend([self.n("Bb", 2, HALF), self.n("D", 3, HALF)])
        notes.extend([self.n("Bb", 2, HALF), self.n("Ab", 2, HALF)])
        notes.extend([self.n("Eb", 2, HALF), self.n("G", 2, HALF)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("G", 2, HALF), self.n("D", 3, HALF)])
        notes.extend([self.n("C", 2, WHOLE)])
        notes.extend([self.n("F", 2, WHOLE)])
        notes.extend([self.n("Bb", 1, WHOLE)])
        return notes
    
    def intro_chords(self):
        return [
            Chord("Bb", "7"), Chord("Bb", "7"),
            Chord("Eb", "7"), Chord("Bb", "7"),
            Chord("G", "m7"), Chord("C", "m7"),
            Chord("F", "7#9"), Chord("Bb", "7"),
        ]
    
    # =========================================================================
    # TRIAD SOLO: Arpeggiated with Scofield angular syncopation
    # =========================================================================
    
    def triad_solo(self):
        items = []
        
        # Bar 1: Bb + C triads (whole step pair - OUTSIDE!)
        # 8+8+8+8+8+8+16 = 64 ✓
        bar1 = [
            self.r(EIGHTH),              # Syncopated start!
            self.n("Bb", 4, EIGHTH),     # Bb arp
            self.n("D", 5, EIGHTH),      
            self.n("F", 5, EIGHTH),      
            self.n("C", 5, EIGHTH),      # C arp (outside!)
            self.n("E", 5, EIGHTH),      
            self.n("G", 5, QUARTER),     # Hold
        ]
        verify(bar1, 1, "TriadSolo")
        items.extend(bar1)
        
        # Bar 2: Db + Eb (tension!)
        # 8+8+8+8+8+8+8+8 = 64 ✓
        bar2 = [
            self.n("Db", 5, EIGHTH),     # Db arp - OUTSIDE!
            self.n("F", 5, EIGHTH),      
            self.n("Ab", 5, EIGHTH),     
            self.n("Bb", 5, EIGHTH),     # Passing
            self.n("Eb", 5, EIGHTH),     # Eb arp
            self.n("G", 5, EIGHTH),      
            self.n("Bb", 5, EIGHTH),     
            self.n("Ab", 5, EIGHTH),     # Landing
        ]
        verify(bar2, 2, "TriadSolo")
        items.extend(bar2)
        
        # Bar 3: Gm + Am (relative minor pair)
        # 8+8+8+16+8+8+8 = 64 ✓
        bar3 = [
            self.n("G", 4, EIGHTH),      # Gm arp
            self.n("Bb", 4, EIGHTH),     
            self.n("D", 5, EIGHTH),      
            self.n("E", 5, QUARTER),     # Am approach - PUSH
            self.n("A", 4, EIGHTH),      # Am arp (outside!)
            self.n("C", 5, EIGHTH),      
            self.n("E", 5, EIGHTH),      
        ]
        verify(bar3, 3, "TriadSolo")
        items.extend(bar3)
        
        # Bar 4: Bb resolution
        # 8+8+8+8+32 = 64 ✓
        bar4 = [
            self.n("F", 5, EIGHTH),      
            self.n("D", 5, EIGHTH),      
            self.n("Bb", 4, EIGHTH),     
            self.n("F", 4, EIGHTH),      
            self.n("Bb", 4, HALF),       # Root hold
        ]
        verify(bar4, 4, "TriadSolo")
        items.extend(bar4)
        
        # Bar 5: Cm + Dm (ii-iii movement)
        # 8+8+8+8+8+8+16 = 64 ✓
        bar5 = [
            self.n("C", 5, EIGHTH),      # Cm arp
            self.n("Eb", 5, EIGHTH),     
            self.n("G", 5, EIGHTH),      
            self.r(EIGHTH),              # Breath
            self.n("D", 5, EIGHTH),      # Dm arp
            self.n("F", 5, EIGHTH),      
            self.n("A", 5, QUARTER),     # Hold
        ]
        verify(bar5, 5, "TriadSolo")
        items.extend(bar5)
        
        # Bar 6: F# + Ab (tritone sub pair - SCOFIELD!)
        # 8+8+8+8+8+8+8+8 = 64 ✓
        bar6 = [
            self.n("F#", 5, EIGHTH),     # F# arp - OUTSIDE!
            self.n("A#", 5, EIGHTH),     
            self.n("C#", 6, EIGHTH),     
            self.n("B", 5, EIGHTH),      # Chromatic
            self.n("Ab", 5, EIGHTH),     # Ab arp
            self.n("C", 6, EIGHTH),      
            self.n("Eb", 6, EIGHTH),     
            self.n("Db", 6, EIGHTH),     
        ]
        verify(bar6, 6, "TriadSolo")
        items.extend(bar6)
        
        # Bar 7: F7 altered area
        # 8+8+8+8+8+8+16 = 64 ✓
        bar7 = [
            self.n("A", 4, EIGHTH),      # F7 3rd
            self.n("Eb", 5, EIGHTH),     # b7
            self.n("G#", 5, EIGHTH),     # #9!
            self.r(EIGHTH),              # Space
            self.n("Gb", 5, EIGHTH),     # Gb arp
            self.n("Bb", 5, EIGHTH),     
            self.n("Db", 6, QUARTER),    # Hold high
        ]
        verify(bar7, 7, "TriadSolo")
        items.extend(bar7)
        
        # Bar 8: Bb final
        # 8+8+8+8+32 = 64 ✓
        bar8 = [
            self.n("Bb", 4, EIGHTH),     
            self.n("D", 5, EIGHTH),      
            self.n("F", 5, EIGHTH),      
            self.n("Ab", 5, EIGHTH),     # b7
            self.n("Bb", 5, HALF),       # Root high - rest
        ]
        verify(bar8, 8, "TriadSolo")
        items.extend(bar8)
        
        return items
    
    def triad_solo_bass(self):
        notes = []
        notes.extend([self.n("Bb", 2, HALF), self.n("C", 2, HALF)])
        notes.extend([self.n("Db", 2, HALF), self.n("Eb", 2, HALF)])
        notes.extend([self.n("G", 2, HALF), self.n("A", 2, HALF)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("C", 2, HALF), self.n("D", 2, HALF)])
        notes.extend([self.n("F#", 2, HALF), self.n("Ab", 2, HALF)])
        notes.extend([self.n("F", 2, WHOLE)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        return notes
    
    def triad_solo_chords(self):
        return [
            Chord("Bb", "7"), Chord("Eb", "7"),
            Chord("G", "m7"), Chord("Bb", "7"),
            Chord("C", "m7"), Chord("F", "7alt"),
            Chord("F", "7#9"), Chord("Bb", "7"),
        ]
    
    # =========================================================================
    # CHORD MELODY: Quartal voicings (stacked 4ths) - Scofield signature
    # =========================================================================
    
    def chord_melody(self):
        items = []
        
        # Bar 1: Bb7 quartal - stacked 4ths!
        bar1 = [
            self.chord([("Eb", 5), ("Bb", 4), ("F", 4), ("C", 4)], HALF),  # 4ths!
            self.chord([("F", 5), ("C", 5), ("G", 4), ("D", 4)], HALF),
        ]
        verify(bar1, 1, "Quartal")
        items.extend(bar1)
        
        # Bar 2: Continue quartal motion
        bar2 = [
            self.chord([("G", 5), ("D", 5), ("A", 4), ("E", 4)], HALF),
            self.chord([("Ab", 5), ("Eb", 5), ("Bb", 4), ("F", 4)], HALF),
        ]
        verify(bar2, 2, "Quartal")
        items.extend(bar2)
        
        # Bar 3: Eb7 quartal
        bar3 = [
            self.chord([("Db", 5), ("Ab", 4), ("Eb", 4), ("Bb", 3)], HALF),
            self.chord([("Eb", 5), ("Bb", 4), ("F", 4), ("C", 4)], HALF),
        ]
        verify(bar3, 3, "Quartal")
        items.extend(bar3)
        
        # Bar 4: Bb7sus with voice leading
        bar4 = [
            self.chord([("F", 5), ("C", 5), ("G", 4), ("D", 4)], WHOLE),
        ]
        verify(bar4, 4, "Quartal")
        items.extend(bar4)
        
        # Bar 5: Gm quartal
        bar5 = [
            self.chord([("D", 5), ("A", 4), ("E", 4), ("B", 3)], HALF),
            self.chord([("F", 5), ("C", 5), ("G", 4), ("D", 4)], HALF),
        ]
        verify(bar5, 5, "Quartal")
        items.extend(bar5)
        
        # Bar 6: Cm7 -> F7
        bar6 = [
            self.chord([("G", 5), ("D", 5), ("A", 4), ("Eb", 4)], HALF),
            self.chord([("Eb", 5), ("A", 4), ("E", 4), ("C", 4)], HALF),  # F7#9 voicing!
        ]
        verify(bar6, 6, "Quartal")
        items.extend(bar6)
        
        # Bar 7: F7alt quartal spread
        bar7 = [
            self.chord([("Gb", 5), ("Db", 5), ("Ab", 4), ("Eb", 4)], HALF),
            self.chord([("E", 5), ("B", 4), ("Gb", 4), ("Db", 4)], HALF),
        ]
        verify(bar7, 7, "Quartal")
        items.extend(bar7)
        
        # Bar 8: Bb7 resolution
        bar8 = [
            self.chord([("F", 5), ("D", 5), ("Ab", 4), ("Bb", 3)], WHOLE),
        ]
        verify(bar8, 8, "Quartal")
        items.extend(bar8)
        
        return items
    
    def chord_melody_bass(self):
        notes = []
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("Eb", 2, WHOLE)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("G", 2, WHOLE)])
        notes.extend([self.n("C", 2, HALF), self.n("F", 2, HALF)])
        notes.extend([self.n("F", 2, WHOLE)])
        notes.extend([self.n("Bb", 1, WHOLE)])
        return notes
    
    def chord_melody_chords(self):
        return [
            Chord("Bb", "7"), Chord("Bb", "7"),
            Chord("Eb", "7"), Chord("Bb", "7sus4"),
            Chord("G", "m7"), Chord("C", "m7"),
            Chord("F", "7alt"), Chord("Bb", "7"),
        ]
    
    # =========================================================================
    # OUTRO: Intensified melody with gritty resolution
    # =========================================================================
    
    def outro_melody(self):
        items = []
        
        # Bar 1: More intense version
        bar1 = [
            self.n("D", 5, EIGHTH),      
            self.n("F", 5, EIGHTH),      
            self.n("Ab", 5, EIGHTH),     
            self.n("A", 5, EIGHTH),      # Chromatic tension!
            self.n("Bb", 5, QUARTER),    
            self.n("G", 5, QUARTER),     
        ]
        verify(bar1, 1, "Outro")
        items.extend(bar1)
        
        # Bar 2: Angular descent
        bar2 = [
            self.n("Bb", 5, EIGHTH),     
            self.n("A", 5, EIGHTH),      # Chromatic
            self.n("Ab", 5, QUARTER),    
            self.n("F", 5, QUARTER),     
            self.n("D", 5, QUARTER),     
        ]
        verify(bar2, 2, "Outro")
        items.extend(bar2)
        
        # Bar 3: Eb7 punch
        bar3 = [
            self.n("Eb", 5, EIGHTH),     
            self.n("G", 5, EIGHTH),      
            self.n("Db", 6, QUARTER),    # High b7
            self.n("Bb", 5, HALF),       
        ]
        verify(bar3, 3, "Outro")
        items.extend(bar3)
        
        # Bar 4: Bb approach
        bar4 = [
            self.r(EIGHTH),              # Anticipation
            self.n("F", 5, EIGHTH),      
            self.n("D", 5, QUARTER),     
            self.n("Bb", 4, HALF),       
        ]
        verify(bar4, 4, "Outro")
        items.extend(bar4)
        
        # Bar 5: Gm with edge
        bar5 = [
            self.n("G", 4, EIGHTH),      
            self.n("Bb", 4, EIGHTH),     
            self.n("D", 5, QUARTER),     
            self.n("F", 5, QUARTER),     
            self.n("G", 5, QUARTER),     
        ]
        verify(bar5, 5, "Outro")
        items.extend(bar5)
        
        # Bar 6: Cm7 deeper
        bar6 = [
            self.n("C", 5, QUARTER),     
            self.n("Eb", 5, QUARTER),    
            self.n("G", 5, QUARTER),     
            self.n("Bb", 5, QUARTER),    
        ]
        verify(bar6, 6, "Outro")
        items.extend(bar6)
        
        # Bar 7: F7#9 grit
        bar7 = [
            self.n("A", 4, EIGHTH),      
            self.n("Eb", 5, EIGHTH),     
            self.n("G#", 5, QUARTER),    # #9!
            self.n("F", 5, HALF),        
        ]
        verify(bar7, 7, "Outro")
        items.extend(bar7)
        
        # Bar 8: Bb7 final
        bar8 = [
            self.chord([("Ab", 5), ("F", 5), ("D", 5), ("Bb", 4)], WHOLE),
        ]
        verify(bar8, 8, "Outro")
        items.extend(bar8)
        
        return items
    
    def outro_bass(self):
        notes = []
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("Eb", 2, WHOLE)])
        notes.extend([self.n("Bb", 2, WHOLE)])
        notes.extend([self.n("G", 2, WHOLE)])
        notes.extend([self.n("C", 2, WHOLE)])
        notes.extend([self.n("F", 2, WHOLE)])
        notes.extend([self.n("Bb", 1, WHOLE)])
        return notes
    
    def outro_chords(self):
        return [
            Chord("Bb", "7"), Chord("Bb", "7"),
            Chord("Eb", "7"), Chord("Bb", "7"),
            Chord("G", "m7"), Chord("C", "m7"),
            Chord("F", "7#9"), Chord("Bb", "7"),
        ]
    
    # =========================================================================
    # MusicXML Export
    # =========================================================================
    
    def create_musicxml(self):
        score = ET.Element("score-partwise", version="4.0")
        
        work = ET.SubElement(score, "work")
        ET.SubElement(work, "work-title").text = self.title
        
        ident = ET.SubElement(score, "identification")
        creator = ET.SubElement(ident, "creator", type="composer")
        creator.text = self.composer
        
        part_list = ET.SubElement(score, "part-list")
        gtr = ET.SubElement(part_list, "score-part", id="P1")
        ET.SubElement(gtr, "part-name").text = "Guitar"
        bass = ET.SubElement(part_list, "score-part", id="P2")
        ET.SubElement(bass, "part-name").text = "Bass"
        
        print("\nVerifying all bars...")
        sections = [
            ("1 - Intro", self.intro_melody(), self.intro_bass(), self.intro_chords()),
            ("2 - Triad Solo", self.triad_solo(), self.triad_solo_bass(), self.triad_solo_chords()),
            ("3 - Quartal CM", self.chord_melody(), self.chord_melody_bass(), self.chord_melody_chords()),
            ("4 - Outro", self.outro_melody(), self.outro_bass(), self.outro_chords()),
        ]
        
        all_guitar = []
        all_bass = []
        all_chords = []
        markers = []
        
        bar_count = 0
        for name, g, b, c in sections:
            markers.append((bar_count, name))
            all_guitar.extend(g)
            all_bass.extend(b)
            all_chords.extend(c)
            bar_count += 8
        
        guitar_part = ET.SubElement(score, "part", id="P1")
        self._write_part(guitar_part, all_guitar, all_chords, markers, True)
        
        bass_part = ET.SubElement(score, "part", id="P2")
        self._write_part(bass_part, all_bass, [], [], False)
        
        rough = ET.tostring(score, encoding="unicode")
        return minidom.parseString(rough).toprettyxml(indent="  ")
    
    def _write_part(self, part, items, chords, markers, is_guitar):
        marker_dict = {m: name for m, name in markers}
        measure_num = 1
        item_idx = 0
        chord_idx = 0
        
        while item_idx < len(items):
            measure = ET.SubElement(part, "measure", number=str(measure_num))
            
            if measure_num == 1:
                attr = ET.SubElement(measure, "attributes")
                ET.SubElement(attr, "divisions").text = str(self.divisions)
                key = ET.SubElement(attr, "key")
                ET.SubElement(key, "fifths").text = str(self.key_fifths)
                ET.SubElement(key, "mode").text = "major"
                time = ET.SubElement(attr, "time")
                ET.SubElement(time, "beats").text = "4"
                ET.SubElement(time, "beat-type").text = "4"
                clef = ET.SubElement(attr, "clef")
                ET.SubElement(clef, "sign").text = "G" if is_guitar else "F"
                ET.SubElement(clef, "line").text = "2" if is_guitar else "4"
                
                if is_guitar:
                    direction = ET.SubElement(measure, "direction", placement="above")
                    dt = ET.SubElement(direction, "direction-type")
                    met = ET.SubElement(dt, "metronome")
                    ET.SubElement(met, "beat-unit").text = "quarter"
                    ET.SubElement(met, "per-minute").text = str(self.tempo)
            
            if is_guitar and (measure_num - 1) in marker_dict:
                direction = ET.SubElement(measure, "direction", placement="above")
                dt = ET.SubElement(direction, "direction-type")
                ET.SubElement(dt, "rehearsal").text = marker_dict[measure_num - 1]
            
            if is_guitar and chord_idx < len(chords):
                c = chords[chord_idx]
                harmony = ET.SubElement(measure, "harmony")
                root = ET.SubElement(harmony, "root")
                if len(c.root) > 1 and c.root[1] in '#b':
                    ET.SubElement(root, "root-step").text = c.root[0]
                    ET.SubElement(root, "root-alter").text = "1" if c.root[1] == '#' else "-1"
                else:
                    ET.SubElement(root, "root-step").text = c.root
                ET.SubElement(harmony, "kind").text = c.to_kind()
                chord_idx += 1
            
            dur_in_bar = 0
            while item_idx < len(items) and dur_in_bar < BAR:
                item = items[item_idx]
                
                if isinstance(item, ChordVoicing):
                    for i, pitch in enumerate(item.pitches):
                        note_elem = ET.SubElement(measure, "note")
                        if i > 0:
                            ET.SubElement(note_elem, "chord")
                        p = ET.SubElement(note_elem, "pitch")
                        step, octave, alter = self._pitch_xml(pitch)
                        ET.SubElement(p, "step").text = step
                        if alter != 0:
                            ET.SubElement(p, "alter").text = str(alter)
                        ET.SubElement(p, "octave").text = str(octave)
                        ET.SubElement(note_elem, "duration").text = str(item.duration)
                        t, dot = self._dur_type(item.duration)
                        ET.SubElement(note_elem, "type").text = t
                        if dot:
                            ET.SubElement(note_elem, "dot")
                    dur_in_bar += item.duration
                    item_idx += 1
                    
                elif isinstance(item, Note):
                    note_elem = ET.SubElement(measure, "note")
                    if item.is_rest:
                        ET.SubElement(note_elem, "rest")
                    else:
                        p = ET.SubElement(note_elem, "pitch")
                        step, octave, alter = self._pitch_xml(item.pitch)
                        ET.SubElement(p, "step").text = step
                        if alter != 0:
                            ET.SubElement(p, "alter").text = str(alter)
                        ET.SubElement(p, "octave").text = str(octave)
                    ET.SubElement(note_elem, "duration").text = str(item.duration)
                    t, dot = self._dur_type(item.duration)
                    ET.SubElement(note_elem, "type").text = t
                    if dot:
                        ET.SubElement(note_elem, "dot")
                    dur_in_bar += item.duration
                    item_idx += 1
            
            measure_num += 1
    
    def _pitch_xml(self, midi):
        octave = (midi // 12) - 1
        pc = midi % 12
        step, alter = MIDI_TO_NOTE[pc]
        return (step, octave, alter)
    
    def _dur_type(self, dur):
        if dur == DOTTED_HALF:
            return ("half", True)
        if dur == DOTTED_QUARTER:
            return ("quarter", True)
        if dur == DOTTED_EIGHTH:
            return ("eighth", True)
        mapping = {WHOLE: "whole", HALF: "half", QUARTER: "quarter",
                   EIGHTH: "eighth", SIXTEENTH: "16th"}
        return (mapping.get(dur, "quarter"), False)
    
    def save(self, path):
        xml = self.create_musicxml()
        lines = xml.split('\n')
        if lines[0].startswith('<?xml'):
            lines = lines[1:]
        
        content = '<?xml version="1.0" encoding="UTF-8"?>\n'
        content += '<!DOCTYPE score-partwise PUBLIC "-//Recordare//DTD MusicXML 4.0 Partwise//EN" '
        content += '"http://www.musicxml.org/dtds/partwise.dtd">\n'
        content += '\n'.join(lines)
        
        with open(path, 'w', encoding='utf-8') as f:
            f.write(content)
        return path


def main():
    print("=" * 60)
    print("CHROMATIC ORBIT x5 - 5-STAR VERSION")
    print("John Scofield Style - UPGRADED")
    print("=" * 60)
    
    out = Path(__file__).parent.parent / "Trio Tunes" / "Alternative_LeadSheets" / "John Scofield - Chromatic Orbit"
    out.mkdir(parents=True, exist_ok=True)
    
    comp = ChromaticOrbitX5()
    path = out / "Chromatic_Orbit_x5.musicxml"
    comp.save(str(path))
    
    print(f"\nSaved: {path}")
    print("\n5-STAR UPGRADES:")
    print("  ✓ More angular chromatic melody")
    print("  ✓ F7#9 altered dominant (Scofield signature)")
    print("  ✓ Tritone sub triad pairs (F# + Ab)")
    print("  ✓ Sophisticated quartal stacking")
    print("  ✓ Grittier funk with syncopation")
    print("=" * 60)


if __name__ == "__main__":
    main()

